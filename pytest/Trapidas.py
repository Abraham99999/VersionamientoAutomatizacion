...
class LTAutomate(unittest.TestCase):
    ...
    def test_transferencias_propias(self):
        driver = self.driver
        print("Driver initiated successfully.  Navigate url")
        driver.get("https://mifel.net/qa-banca-electronica-pf/")
        time.sleep(10)
        wait = WebDriverWait(driver, 10)

        element = wait.until(EC.presence_of_element_located(
            (By.XPATH, "/html/body/div/div/div[2]/div/div[1]/form/span/div/div[1]/div/input")))
        ...
        continuar_button = wait.until(
            EC.presence_of_element_located((By.XPATH, "//button[@type='button'][contains(.,'Continuar')]")))
        continuar_button.click()

        llavero = wait.until(EC.presence_of_element_located(
            (By.XPATH, "/html/body/div/div/div[2]/div/div/form/div/div[1]/span/div/div[1]/div/input")))
        ...
        boton_autorizar = wait.until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div/div/div[2]/div/div/form/div/div[2]/button")))
        ...
        hamburguesa.click();
        allure.attach(driver.get_screenshot_as_png(), name="Seleccionar menú hamburguesa",
                      attachment_type=AttachmentType.PNG)

        opctransferencia = wait.until(EC.element_to_be_clickable((By.XPATH,
                                                                  "//button[contains(@translate-en,'My transfers') or contains(@translate-es,'Mis transferencias')]")))
        opctransferencia.click();
        allure.attach(driver.get_screenshot_as_png(), name="Seleccionar Transferencia",
                      attachment_type=AttachmentType.PNG)

        opcrealizartran = wait.until(EC.visibility_of_element_located((By.LINK_TEXT, "Realizar una transferencia")))
        allure.attach(driver.get_screenshot_as_png(), name="Seleccionar Realizar una transferencia",
                      attachment_type=AttachmentType.PNG)
        opcrealizartran.click()

        driver.implicitly_wait(1)
        montos = ["0.70", "0.80", "0.90"]
        for i in range(len(montos)):
            ...
            boton_propias = wait.until(EC.element_to_be_clickable(
                (By.XPATH, "/html/body/div[2]/section/div/div[2]/div/div[2]/div[2]/div[1]/button[2]")))
            # Haz clic en el botón "Propias"
            allure.attach(driver.get_screenshot_as_png(), name="Seleccionar Propias",
                          attachment_type=AttachmentType.PNG)
            boton_propias.click()
            ...
            selCuenta = wait.until(
                EC.presence_of_element_located((By.XPATH, "//button[contains(., 'Cuenta Básica Nómina')]")))
            # Hacer clic en el elemento
            selCuenta.click()
            # Esperar a que aparezaca el botón Continuar
            elemento_continuar = wait.until(
                EC.presence_of_element_located((By.XPATH, "//button[contains(., 'Continuar')]")))

            # Ingresar monto
            inputMonto = wait.until(EC.presence_of_element_located((By.XPATH, "//input[@name='Monto a transferir']")))
            inputMonto.send_keys(montos[i])
            allure.attach(driver.get_screenshot_as_png(), name="Ingresar Monto" + montos[i],
                          attachment_type=AttachmentType.PNG)
            driver.implicitly_wait(1)
            # Captura btnContinuar
            boton_continuar = wait.until(EC.presence_of_element_located(
                (By.XPATH, "//button[@class='mifel-btn transfer-amount__button mifel-btn-primary mifel-btn-block']")))
            ...
            exitosa = wait.until(
                EC.text_to_be_present_in_element((By.XPATH, '//*[@id="voucher"]/section[1]/section/span'),
                                                 '¡Transferencia exitosa!'))
            allure.attach(driver.get_screenshot_as_png(), name="Transferencia Exitosa",
                          attachment_type=AttachmentType.PNG)
            time.sleep(4)
            ...
            btnnuevatransfer = wait.until(
                EC.element_to_be_clickable((By.XPATH, "//button[contains(text(), 'Nueva transferencia')]")))
